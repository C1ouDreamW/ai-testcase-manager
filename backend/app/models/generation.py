...
class GenerationTask(Base):
    ...
    strategy: Mapped[str] = mapped_column(
        String(50), default="detailed"
    )  # smoke, detailed
    strategy_config: Mapped[str] = mapped_column(
        Text, default=""
    )  # JSON: strategy, specialist_skills
    status: Mapped[str] = mapped_column(String(20), default="pending")
    # pending → structuring → confirmed → generating → reviewing → completed / failed
    progress: Mapped[int] = mapped_column(Integer, default=0)
    stage: Mapped[str] = mapped_column(
        String(100), default=""
    )  # 当前阶段提示，如「生成用例 2/5：xxx」
    error_message: Mapped[str] = mapped_column(Text, default="")
    tokens_used: Mapped[int] = mapped_column(Integer, default=0)
    is_eval: Mapped[bool] = mapped_column(
        default=False
    )  # 评测运行产生的任务，业务列表默认过滤
    knowledge_refs: Mapped[str] = mapped_column(
        Text, default=""
    )  # JSON: {item_id: [{title, heading, score}]}，RAG 溯源
    ...
    def review_stats(self) -> dict:
        ...
        edited_adopted = sum(
            1 for d in drafts if d.review_status == "adopted" and d.was_edited
        )
...
class GeneratedCaseDraft(Base):
    ...
    review_status: Mapped[str] = mapped_column(
        String(20), default="pending"
    )  # pending, adopted, rejected, edited
    was_edited: Mapped[bool] = mapped_column(
        default=False
    )  # 是否被人工编辑过（采纳后 review_status 会覆盖 edited，用此字段保留信号）
    ...
    judge_score: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )  # AI Judge 综合分 1~5，None 表示未评
    judge_issues: Mapped[str] = mapped_column(
        Text, default=""
    )  # JSON: 各维度分、幻觉标记与理由
...
class QualityReport(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    task_id: Mapped[int] = mapped_column(
        ForeignKey("generation_tasks.id"), unique=True, nullable=False
    )
