...
├── autotest/             # pytest + Playwright 自动化测试与 Allure 报告
└── .env.example
```
...
项目内置独立自动化测试工程，使用 pytest + requests 完成 API 测试，使用 Playwright 完成核心 UI 流程测试，并通过 Allure 生成可视化报告。测试环境使用独立 SQLite、ChromaDB 和 LLM Mock 模式，不污染开发数据。

- API 测试：74 条
- UI 测试：20 条
- 全量回归：94 条全部通过，通过率 100%
- [查看完整自动化测试报告（CSDN）](https://blog.csdn.net/m0_55168214/article/details/162934606)
...
- **测试工程化** — 94 条 API/UI 自动化用例 + 隔离测试环境 + Allure 可视化报告
